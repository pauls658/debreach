    ...
    int length;
{
    ...
    if (zmemcmp(s->window + match,
        ...
        ffprintf(stderr, stderr, " start %u, match %u, length %d\n",
                start, match, length);
        do {
            ffprintf(stderr, stderr, "%c%c", s->window[match++], s->window[start++]);
        } while (--length != 0);
        z_error("invalid match");
    }
    if (z_verbose > 1) {
        ffprintf(stderr, stderr,"\\[%d,%d]", start-match, length);
	...
	unsigned len;
{
	...
	if (needed > state->taint_cap) {
		...
		new_size = state->taint_cap*sizeof(int);
		state->tainted_brs = (int *) realloc(state->tainted_brs, new_size);
...
// Make a max_match variable
        ...
        len = scan > strend ? max_match : max_match - (int)(strend - scan);
        scan = strend - max_match;
