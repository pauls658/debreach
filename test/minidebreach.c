...
void file_compress_brs    OF((char  *file, char *mode, int *brs, unsigned int taint_len));
...
int* get_byte_ranges(const char *buf, unsigned int buf_len, char **unsafe) {
	if (unsafe == NULL) {
		fprintf(stdout, "unsafe was null. wat\n");
		return NULL;
	}
	int array_size = 20;
	int* brs = (int*) malloc(sizeof(int)*array_size);
	...
	char **unsafe;
{
...
#if 0
	int *brs = get_byte_ranges((char*)buf, len, unsafe);
#ifdef BRS_ONLY
    ...
    strm->avail_in = len;
	// declare_unsafe inits the tainted_brs
	if (unsafe != NULL)
		declare_unsafe(strm, unsafe);
    do {
        strm->next_out = out;
...
void file_compress_brs(file, mode, brs, taint_len)
    char  *file;
    char  *mode;
	int   *brs;
    ...
    char *argv[];
{
    ...
    if (uncompr) {
        ...
        if (copyout) {
			...
			if (taint != NULL)
            	file_compress_brs(argv[argc - 1], outmode, taint, taint_len);
			else
