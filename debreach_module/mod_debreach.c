...
void align_brs(request_rec *r, z_stream *z, int *brs, int len) {
	int i;
	for (i = 0; i < len; i++) { 
		brs[i] -= ((int) z->total_in);
		if (brs[i] < 0) {
...
#ifdef BRS_DEBUG
void apache_log_int_arr(request_rec *r, int* arr, int len) {
	...
	int i, written = 0;
	for (i = 0; i < len; i++) {
		written += snprintf(buf + written, BUF_SIZE - written, "%d,", arr[i]);
		if (written >= BUF_SIZE)
			break;
	}
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, APLOGNO(01386)
        "written: %d, size: %d, arr: %s",
		written, len, buf);
}
#endif

int buffer_br(ap_filter_t *f, int start, int end) {
	debreach_ctx *ctx = f->ctx;
	request_rec *r = f->r;

	if (!ctx->brs_buf) {
		// we allocated the ctx, but did not allocate brs_buf
		// something bad probably happend
		return -1;
	}

	if (!(ctx->brs_buf_cap - ctx->brs_buf_len >= 2)) {
		// need more space
		int *temp = ctx->brs_buf;
		ctx->brs_buf_cap *= 2;
		// sanity check
		if (ctx->brs_buf_cap < ctx->brs_buf_len - 2) {
    		ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, APLOGNO(01386)
        		"Double the buf cap didn't give us enough space? cap: %d, cur len: %d",
				ctx->brs_buf_cap, ctx->brs_buf_len);
			return -1;
		}

		ctx->brs_buf = apr_pcalloc(r->pool, ctx->brs_buf_cap*sizeof(*ctx->brs_buf));
		// apache does not believe in freeing memory lol otherwise I would free it the
		// old memory here
		memcpy(ctx->brs_buf, temp, ctx->brs_buf_len*sizeof(*temp));
	}

	ctx->brs_buf[ctx->brs_buf_len] = start;
	ctx->brs_buf[ctx->brs_buf_len + 1] = end;
	ctx->brs_buf_len += 2;
	return 0;
}
                                       ...
                                       apr_bucket_brigade *bb)
{
    ...
    if (!ctx || !ctx->initd) {
		...
		if (!ctx) {
			...
			ctx->brs_buf_len = 0;
			ctx->brs_buf_cap = 0;
		} // else we did the memory alloc in mod_debreach_taint_brs, but never
		  // initialized the rest of the filter's state
...
//#ifdef BRS_DEBUG
//			apache_log_int_arr(r, brs, *brs_len);
//#endif
//			if (*brs_len != 0) {
//				align_brs(r, &(ctx->stream), brs, *brs_len);
//				// + 2 cause we still use that gosh darn double null term
//				taint_brs(&(ctx->stream), brs, (*brs_len) + 2);
//			}
//			apr_table_unset(r->notes, DEBREACH_ARR_KEY);
//			apr_table_unset(r->notes, DEBREACH_LEN_KEY);
//
//#ifdef VDEBUG
