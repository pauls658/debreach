...
unsigned int* get_byte_ranges(const char *buf, unsigned int buf_len, char **unsafe) {
	...
	int array_size = 20;
	unsigned int* brs = (unsigned int*) malloc(sizeof(unsigned int)*array_size);
	// TODO: alloc error handling
	brs[0] = 0;
	brs[1] = 0;
	unsigned int buf_i, br_i = 0;
	char **temp;
	for (buf_i = 0; buf_i < buf_len; buf_i++) {
		...
		for (temp = unsafe; *temp != NULL; temp++) {
			...
			short match = 1;
			while (unsafe_str[end_i] != '\0' && buf_i + end_i < buf_len) {
				if (unsafe_str[end_i] != buf[buf_i + end_i]) {
					match = 0;
					break;
				}
				end_i++;
			...
			if (match == 1) {
				...
				// First check if buf_i is still in the last byte range
				if (buf_i <= brs[br_i + 1]) {
					if (buf_i + end_i > brs[br_i + 1]) {
						// buf_i hasn't moved out of the last byte range yet, and the new br extends
						// out of the old one
						// don't make a new byte range, just extend the last one
						brs[br_i + 1] = buf_i + end_i; 
					}
				} else {
					...
					if (br_i + 4 > array_size - 2) {
						...
						brs = (unsigned int*) realloc(brs, array_size);
					}
					// TODO: alloc error handling
					brs[br_i] = buf_i;
					brs[br_i + 1] = buf_i + end_i;
